...
best_param
```
**BEST MODEL ACCURACY = 0.7336347**

**Area under the roc curve = ___________________________________ **
...
rf_grid <- grid_regular(
  mtry(finalize(mtry(), lending_training %>% select(-Class))),
  min_n(0, 100),
  levels = 3)
...
rf_tune <- 
  rf_workflow %>% 
  tune_grid(finalize(mtry(), lending_training %>% select(-Class)), 
    resamples = lending_cv,
    grid = min_n_grid_new,
    control = ctrl_grid)
