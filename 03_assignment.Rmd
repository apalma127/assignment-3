...
```{r}
Decl <- gutenberg_works(title == "The Declaration of Independence of the United States of America")

Decl
```
```{r}
books <- gutenberg_works() 
D_of_I <- gutenberg_download(1, mirror = "http://mirrors.xmission.com/gutenberg/") 
```
...
head(common_words_D_of_I, 15)
```


```{r}
common_words_D_of_I_sub <- D_of_I %>%
  unnest_tokens(word, text) %>% 
...
```{r}
new_D_of_I_common <- head(common_words_D_of_I_sub, 15)
new_D_of_I_common
```
